...
HAL_StatusTypeDef I2CDevice::innerIsReady()
{
	...
	HAL_StatusTypeDef status = logStatus(
			HAL_I2C_IsDeviceReady(_hi2c, _address, 3, 1000)
	);
	logText("\n");
	return status;
}
...
void I2CDevice::SetRegister(uint8_t reg, uint8_t value)
{
	write(reg, &value, 1);
}

uint8_t I2CDevice::GetRegister(uint8_t reg)
{
	uint8_t value = 0;
	read(reg, &value, 1);
	return value;
...
void I2CDevice::SetBitRegister(uint8_t reg, uint8_t bit)
{
	if (bit > 7) bit = 7;
...
void I2CDevice::ResetBitRegister(uint8_t reg, uint8_t bit)
{
	if (bit > 7) bit = 7;
	uint8_t tmp = GetRegister(reg);
	tmp &= ~(1 << bit);
...
void I2CDevice::BitRegister(uint8_t reg, uint8_t bit, uint8_t value)
{
	if (value)
	{
